...
def get_mean_std(file_path, num_images):
    # Goes through the training data folder to calculate the mean and std of the images in it
    # Arguments:
    #   file_path: path to the train folder. This should include /train/
    #   num_images: number of images to use in calculating the mean and standard deviation
    import os
    ...
    # Conver the string file_path to a Path
    file_path = Path(file_path)

    # Get paths to all the images in the folders and add the paths to a list
    image_paths = []
    ...
    # Shuffle the image paths and take a subset to get a random subset of images
    random.shuffle(image_paths)
    if num_images < len(image_paths):
        image_paths = image_paths[0:num_images]

    # Import the images, convert them into 2D numpy arrays and append them to the 3D array
    # Get a starter array
    img = Image.open(image_paths.pop())
    # Convert the image into a numpy array
    img = np.array(img)
    # Divide by the max value to get range 0 to 1
    # Start the image array to store all the images
    image_array = (img - np.min(img)) / np.ptp(img)
    ...
    # Get the mean and standard deviation of the image array
...
def process_image_BW(file_path, mean, std, transform = None):
    # Function loads a black and white .png image and transforms it into
    # a tensor suitable for use with pytorch
