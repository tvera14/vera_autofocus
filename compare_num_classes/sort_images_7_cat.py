...
#. the images for import and use with Pytorch using ImageFolder and wormDataset.
...
class_range = 6 # Number of steps for classes other than acceptable

# Keep track of how many images and classes have been generated
image_counter = [0, 0, 0, 0, 0, 0, 0]
	...
	for stack in worm_dir.glob('* focus'):
		...
		if q.exists():
			...
			for image in stack_dir.glob('*.png'):
				...
				if distance < (-1 * (acceptable_range + 2 * class_range)):
					class_folder = sorted_dir / '0'
					image_counter[0] += 1
				elif distance < (-1 * (acceptable_range + class_range)):
					class_folder = sorted_dir / '1'
					image_counter[1] += 1
				elif distance <= (-1 * acceptable_range):
					class_folder = sorted_dir / '2'
					image_counter[2] += 1
				elif distance <= acceptable_range:
					class_folder = sorted_dir / '3'
					image_counter[3] += 1
				elif distance < (acceptable_range + class_range):
					class_folder = sorted_dir / '4'
					image_counter[4] += 1
				elif distance < (acceptable_range + 2 * class_range):
					class_folder = sorted_dir / '5'
					image_counter[5] += 1
				else:
					class_folder = sorted_dir / '6'
					image_counter[6] += 1
